...
```{r}
my_manual_simulator <- function(theta, fidelity = "high", n_rep = NULL, ...) {
  # Fallback for calls outside BATON; keep in sync with your fidelity_levels
  if (is.null(n_rep)) {
    n_rep <- switch(fidelity, low = 2000, med = 4000, high = 10000)
  }
...
```{r speed-benchmark, eval = requireNamespace("microbenchmark", quietly = TRUE)}
# Speed overhead (requires the suggested package microbenchmark)
library(microbenchmark)

# Benchmark Welford against storing everything, on a trivial sampler
...
**Reading this honestly**: on this toy the sampler does almost no work (two
random draws), so the Welford accumulator dominates the runtime and the
`welford` timing comes out severalfold slower than the vectorized naive
version. That ratio is an artifact of the trivial sampler, not a property of
the algorithm. In a real trial simulator, one replicate simulates an entire
trial (enrollment, interim analyses, posterior computations), which costs
orders of magnitude more than the accumulator's few arithmetic operations
per sample. Relative to any simulator worth optimizing, the overhead is
negligible; what you keep is the 5,000x memory reduction and the variance
estimates that make the heteroskedastic GP possible.

# Real-World Example

Here's a realistic clinical trial simulator. Two things matter about its
structure. First, it simulates under **both hypotheses** inside the
evaluator: power comes from replicates with the assumed true effect
(`delta`), type I error from replicates with zero effect. Second, the
parameters BATON searches over are **design knobs only** (sample size and
the futility rule); the true treatment effect is a fixed assumption, never a
...
adaptive_trial_simulator <- function(theta,
  ...
  delta <- 0.5  # assumed true effect under the alternative (fixed, not searched)

  n_per_arm <- round(theta$n_per_arm)          # design knob
  fut_p     <- theta$futility_threshold        # design knob
  n_interim <- max(2L, round(n_per_arm / 2))   # interim at 50% enrollment

  # One replicate under a given true effect: returns reject indicator and N
  run_one <- function(effect) {
    control   <- rnorm(n_per_arm, mean = 0, sd = 1)
    treatment <- rnorm(n_per_arm, mean = effect, sd = 1)

    # Interim analysis: stop for futility if the interim p-value is large
    interim_p <- t.test(treatment[1:n_interim], control[1:n_interim])$p.value
    if (interim_p > fut_p) {
      c(reject = 0, N = 2 * n_interim)
    } else {
      final_p <- t.test(treatment, control)$p.value
      c(reject = as.numeric(final_p < 0.025), N = 2 * n_per_arm)
    }
  }

  result <- welford_mean_var(
    sample_fn = function(i, theta) {
      alt  <- run_one(delta)  # alternative: power and E[N]
      null <- run_one(0)      # null: type I error
      # EN here is expected N under the alternative; use null["N"] instead to
      # minimize E[N] under the null (a design decision, see the README)
      c(power = unname(alt["reject"]),
        type1 = unname(null["reject"]),
        EN    = unname(alt["N"]))
    },
    n_samples = n_rep,
    theta = theta
...
# Quick check at low fidelity
check <- adaptive_trial_simulator(
  theta = list(n_per_arm = 50, futility_threshold = 0.5),
  fidelity = "low",
  seed = 42,
  n_rep = 500  # small direct call for display; BATON would pass fidelity_levels
)
print(check)
```

```{r eval=FALSE}
# Calibrate the design knobs (not evaluated at build time: slow)
bounds <- list(
  n_per_arm = c(30, 100),
  futility_threshold = c(0.2, 0.8)
)
...
fit <- bo_calibrate(
  ...
  bounds = bounds,
  objective = "EN",
  constraints = list(power = c("ge", 0.8), type1 = c("le", 0.10)),
  integer_params = "n_per_arm",
  n_init = 10,
  budget = 30
)
```
...
- Memory: 64 bytes vs 320 KB (5,000x reduction)
- Speed: negligible overhead relative to any real simulator (the accumulator
  costs a few arithmetic operations per replicate; only on a trivial toy
  sampler is it visible, as the benchmark above shows)
...
This skeleton matches the README's "Minimal Template" section, which also
shows the surrounding `bo_calibrate()` call.
...
my_sim <- function(theta, fidelity = "high", seed = NULL, n_rep = NULL, ...) {
  ...
  result <- welford_mean_var(
    sample_fn = function(i, theta) {
      # REPLACE with one replicate of YOUR design; return named metrics,
      # e.g. c(power = ..., type1 = ..., EN = ...)
      stop("REPLACE: simulate one replicate of your design")
    },
    n_samples = n_rep,
    theta = theta
